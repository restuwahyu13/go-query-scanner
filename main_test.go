...
func TestQueryScan(t *testing.T) {
	...
	t.Run("Shoud be a valid query string success", func(t *testing.T) {
		err := QueryScan(queryStr, &queryDest)
		t.Log(err)
...
func BenchmarkQueryScan(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		_ = QueryScan(queryStr, &queryDest)
	}
}

func BenchmarkQueryScanLargePayload(b *testing.B) {
	values := url.Values{}
	for i := 0; i < 100; i++ {
...
func BenchmarkQueryScanWithComplexJSON(b *testing.B) {
	values := url.Values{}
